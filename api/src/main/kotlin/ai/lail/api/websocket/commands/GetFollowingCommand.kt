...
class GetFollowingCommand(body: CommandContent) : WebSocketCommand(body) {
    override val event: WebSocketEvent = WebSocketEvent.GetFollowing
    ...
    override fun handle(response: WebSocketResponse, context: ApplicationContext) {
        ...
        try {
            ...
            val following = followerService.findById(user.id).following
            val users = userService.getManyUserDto(following)

            content.put("message", "Got followed users")
            content.putPOJO("result", users)
