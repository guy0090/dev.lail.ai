...
class UnfollowUserCommand(body: CommandContent) : WebSocketCommand(body) {
    override val event: WebSocketEvent = WebSocketEvent.FollowUser
    ...
    override fun handle(response: WebSocketResponse, context: ApplicationContext) {
        ...
        try {
            ...
            followerService.unfollow(user.id, targetId)
            content.put("message", "Unfollowed user $targetId")
